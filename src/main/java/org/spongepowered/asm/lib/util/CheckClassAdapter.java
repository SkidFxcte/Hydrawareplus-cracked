...
public class CheckClassAdapter extends ClassVisitor
{
    ...
    public static void main(final String[] args) throws Exception {
        ...
        if (args[0].endsWith(".class")) {
            cr = new ClassReader((InputStream)new FileInputStream(args[0]));
        }
        else {
    ...
    public static void verify(final ClassReader cr, final ClassLoader loader, final boolean dump, final PrintWriter pw) {
        ...
        for (int j = 0; j < methods.size(); ++j) {
            ...
            final SimpleVerifier verifier = new SimpleVerifier(Type.getObjectType(cn.name), syperType, (List)interfaces, (cn.access & 0x200) != 0x0);
            final Analyzer<BasicValue> a = (Analyzer<BasicValue>)new Analyzer((Interpreter)verifier);
            if (loader != null) {
    ...
    static void printAnalyzerResult(final MethodNode method, final Analyzer<BasicValue> a, final PrintWriter pw) {
        final Frame<BasicValue>[] frames = (Frame<BasicValue>[])a.getFrames();
        ...
        for (int j = 0; j < method.tryCatchBlocks.size(); ++j) {
            method.tryCatchBlocks.get(j).accept((MethodVisitor)mv);
