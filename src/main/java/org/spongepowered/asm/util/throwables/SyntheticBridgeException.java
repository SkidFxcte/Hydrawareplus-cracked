...
public class SyntheticBridgeException extends MixinException
{
    ...
    public void printAnalysis(final IMixinContext context, final MethodNode mda, final MethodNode mdb) {
        ...
        printer.add().kv("Method", (Object)(this.name + this.desc)).kv("Problem Type", (Object)this.problem).add().hr();
        final String merged = (String)Annotations.getValue(Annotations.getVisible(mda, (Class)MixinMerged.class), "mixin");
    ...
    private PrettyPrinter printMethod(final PrettyPrinter printer, final MethodNode method) {
        ...
        while (iter.hasNext()) {
            printer.kv((index == this.index) ? ">>>>" : "", (Object)Bytecode.describeNode((AbstractInsnNode)iter.next()));
            ++index;
        }
        return printer.add();
    }
    ...
    private PrettyPrinter printProblem(final PrettyPrinter printer, final IMixinContext context, final MethodNode mda, final MethodNode mdb) {
        ...
        switch (this.problem) {
            ...
            case BAD_LOAD: {
                ...
                final ListIterator<AbstractInsnNode> ia = (ListIterator<AbstractInsnNode>)mda.instructions.iterator();
                final ListIterator<AbstractInsnNode> ib = (ListIterator<AbstractInsnNode>)mdb.instructions.iterator();
                ...
                while (ia.hasNext() && ib.hasNext()) {
                    ...
                    if (na instanceof VarInsnNode && nb instanceof VarInsnNode) {
                        ...
                        final Type ta = (va.fd_int_2 > 0) ? argsa[va.fd_int_2 - 1] : target;
                        final Type tb = (vb.fd_int_2 > 0) ? argsb[vb.fd_int_2 - 1] : target;
                        printer.kv("Target " + index, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName((AbstractInsnNode)va), va.fd_int_2, ta });
                        printer.kv("Incoming " + index, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName((AbstractInsnNode)vb), vb.fd_int_2, tb });
                        if (ta.equals((Object)tb)) {
            ...
            case BAD_CAST: {
                ...
                printer.kv("Target type", (Object)ta2);
                printer.kv("Incoming type", (Object)tb2);
                printer.kv("Common supertype", (Object)ClassInfo.getCommonSuperClassOrInterface(ta2, tb2)).add();
                break;
            }
