...
class MixinPreProcessorStandard
{
    ...
    protected void prepareShadow(final MixinInfo.MixinMethodNode mixinMethod, final ClassInfo.Method method) {
        final AnnotationNode shadowAnnotation = Annotations.getVisible((MethodNode)mixinMethod, (Class<? extends Annotation>)Shadow.class);
        ...
        if (mixinMethod.name.startsWith(prefix)) {
            Annotations.setVisible((MethodNode)mixinMethod, (Class<? extends Annotation>)MixinRenamed.class, "originalName", mixinMethod.name);
    ...
    protected void attachMethods(final MixinTargetContext context) {
        ...
        while (iter.hasNext()) {
            ...
            if (!this.validateMethod(context, mixinMethod)) {
                ...
                context.addMixinMethod((MethodNode)mixinMethod);
            }
    ...
    protected boolean attachAccessorMethod(final MixinTargetContext context, final MixinInfo.MixinMethodNode mixinMethod, final SpecialMethod type) {
        final AnnotationNode annotation = mixinMethod.getVisibleAnnotation((Class)type.annotation);
        if (annotation == null) {
            return false;
        }
        ...
        if (MixinEnvironment.getCompatibilityLevel().isAtLeast(MixinEnvironment.CompatibilityLevel.JAVA_8) && method.isStatic()) {
            ...
            final String uniqueName = context.getUniqueName((MethodNode)mixinMethod, true);
        ...
        context.addAccessorMethod((MethodNode)mixinMethod, type.annotation);
        return true;
    }
    ...
    protected boolean attachSpecialMethod(final MixinTargetContext context, final MixinInfo.MixinMethodNode mixinMethod, final SpecialMethod type) {
        ...
        if (target == null) {
            ...
            if (target == null) {
                throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("%s method %s in %s was not located in the target class %s. %s%s", type, mixinMethod.name, this.mixin, context.getTarget(), context.getReferenceMapper().getStatus(), getDynamicInfo((MethodNode)mixinMethod)));
            }
        ...
        if (!Bytecode.compareFlags((MethodNode)mixinMethod, target, 8)) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("STATIC modifier of %s method %s in %s does not match the target", type, mixinMethod.name, this.mixin));
        }
    ...
    private void conformVisibility(final MixinTargetContext context, final MixinInfo.MixinMethodNode mixinMethod, final SpecialMethod type, final MethodNode target) {
        ...
        final Bytecode.Visibility visMethod = Bytecode.getVisibility((MethodNode)mixinMethod);
        ...
        if (visMethod == Bytecode.Visibility.PRIVATE) {
            ...
            context.addUpgradedMethod((MethodNode)mixinMethod);
    ...
    protected void checkMethodNotUnique(final ClassInfo.Method method, final SpecialMethod type) {
        if (method.isUnique()) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("%s method %s in %s cannot be @Unique", type, method.getName(), this.mixin));
        }
    }
    ...
    protected void checkMixinNotUnique(final MixinInfo.MixinMethodNode mixinMethod, final SpecialMethod type) {
        if (this.mixin.isUnique()) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("%s method %s found in a @Unique mixin %s", type, mixinMethod.name, this.mixin));
        }
    }
    ...
    protected boolean attachUniqueMethod(final MixinTargetContext context, final MixinInfo.MixinMethodNode mixinMethod) {
        ...
        if (Bytecode.getVisibility((MethodNode)mixinMethod).ordinal() < Bytecode.Visibility.PUBLIC.ordinal()) {
            final String uniqueName = context.getUniqueName((MethodNode)mixinMethod, false);
        ...
        final AnnotationNode unique = Annotations.getVisible((MethodNode)mixinMethod, Unique.class);
        if (unique == null || !Annotations.getValue(unique, "silent", Boolean.FALSE)) {
            if (Bytecode.hasFlag((MethodNode)mixinMethod, 64)) {
                try {
                    Bytecode.compareBridgeMethods(target, (MethodNode)mixinMethod);
                    ...
                    if (this.verboseLogging || this.env.getOption(MixinEnvironment.Option.DEBUG_VERIFY)) {
                        ex.printAnalysis((IMixinContext)context, target, (MethodNode)mixinMethod);
                    }
    ...
    protected void attachFields(final MixinTargetContext context) {
        ...
        while (iter.hasNext()) {
            ...
            final AnnotationNode shadow = Annotations.getVisible(mixinField, (Class<? extends Annotation>)Shadow.class);
            ...
            if (!this.validateField(context, mixinField, shadow)) {
                ...
                if (target == null) {
                    ...
                    if (target == null) {
                        throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("Shadow field %s was not located in the target class %s. %s%s", mixinField.name, context.getTarget(), context.getReferenceMapper().getStatus(), getDynamicInfo(mixinField)));
                    }
    ...
    protected void transformMethod(final MethodInsnNode methodNode) {
        ...
        if (owner == null) {
            throw new Class18((Throwable)new ClassNotFoundException(methodNode.owner.replace('/', '.')));
        }
    ...
    protected void transformField(final FieldInsnNode fieldNode) {
        ...
        if (owner == null) {
            throw new Class18((Throwable)new ClassNotFoundException(fieldNode.owner.replace('/', '.')));
        }
    ...
    protected static String getDynamicInfo(final MethodNode method) {
        return getDynamicInfo("Method", Annotations.getInvisible(method, (Class<? extends Annotation>)Dynamic.class));
    }
    
    protected static String getDynamicInfo(final FieldNode method) {
        return getDynamicInfo("Field", Annotations.getInvisible(method, (Class<? extends Annotation>)Dynamic.class));
    }
